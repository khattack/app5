...
def get_timeframe_start(timeframe):
    ...
    elif timeframe == "weekly_anchored":
        # Weekly anchored candles start at the nearest Monday to the 1st of the month at 05:00 PKT
        first_of_month = now.replace(day=1, hour=5, minute=0, second=0, microsecond=0)
        if first_of_month.weekday() == 0:  # If the 1st is a Monday
            return first_of_month
        elif first_of_month.weekday() <= 3:  # If the 1st is Tuesday to Thursday
            return first_of_month - timedelta(days=first_of_month.weekday())
        else:  # If the 1st is Friday to Sunday
            return first_of_month + timedelta(days=(7 - first_of_month.weekday()))
...
if st.button("Auto Detect"):
    ...
    timeframes = [
        ("Monthly", "monthly"),
        ("Weekly", "weekly"),
        ("Weekly Anchored", "weekly_anchored"),
        ("Daily", "daily"),
        ("4-Hourly", "4hourly"),
        ("1-Hourly", "1hourly"),
        ("15-Minute", "15min"),
